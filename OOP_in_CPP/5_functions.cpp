...
/*
    With 'inline', when the program is compiled, the function body is actually inserted into the
...
void repchar2(char = '*', int = 45); // declaration
...
/*
    Remember that missing arguments must be the trailing arguments —those at the end of the argument list.
*/
...
/*
    • The next kind of variables is 'global'. 
...
*/
/* Initialization:
    When static variables are initialized, the initialization takes place only once (if not initialized, they're assigned 0).
*/
...
int &setx()
{
    ...
    return x; // Must be a variable with a wider scope (ex. global).
}
...
/*
    ...
    >> You want to pass an argument by reference for efficiency,
        but you want a guarantee that the function could not modify it.
*/
